...
if ( $matrix_core_build eq "true" && $status == 0 )
{
    ...
    {
        $cmd = "path.bat && make matrix_32mb_host_release";
    }
    else
    {
        $cmd = "path.bat && make matrix_32mb_host";
    }
    print "[INFO] $cmd\n";
    ...
    if ( $ret != 0 )
    {
        ...
        {
            $status = 1;
            Utilities::record_log( "$log_dir\\$ENV{BUILD_TAG}.failed", "32M ARM MATRIX BUILD FAILED: $ENV{BUILD_URL}console" );
            print "[ERROR] Meet error when build 32M ARM MATRIX.\n";
            exit 1;
        }
    }
...
if ( $status == 0 )
{
    ...
    if ( $release_type eq "R" )
    {
        $cmd = "path.bat && make host_32mb_release";
    }
    else
    {
    ...
    if ( $ret != 0 )
    {
        ...
        Utilities::record_log( "$log_dir\\$ENV{BUILD_TAG}.failed", "host_32mb BUILD FAILED: $ENV{BUILD_URL}console" );
        exit 1;
    }
}

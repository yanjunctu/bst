...
mkpath("$view_root_kw\\bahama\\code\\annofile", 1);

if ( $status == 0 )
{
    system("echo ==== %date% %time% ====");
    chdir("$view_root_kw\\bahama");
    system("cd");
    $cmd = "code\\path.bat && make matrix SCM_BUILD=Y";
    print "[INFO] $cmd\n";
...
if ( $status == 0 )
{
    ...
    chdir("$view_root_kw\\bahama");
    system("cd");
    $cmd = "code\\path.bat && make arm_all SCM_BUILD=Y";
    print "[INFO] $cmd\n";
    my $ret = system("$cmd 2>&1");
    print "[DEBUG] $ret\n";
    if ( $ret != 0 )
...
if ( $status == 0 )
{
    ...
    chdir("$view_root_kw\\bahama");
    system("cd");
    $cmd = "code\\path.bat && make dsp_all SCM_BUILD=Y";
    print "[INFO] $cmd\n";
    my $ret = system("$cmd 2>&1");
    print "[DEBUG] $ret\n";
    if ( $ret != 0 )
...
# Run KW
##################################################
my $result = StepCommon::run_kw( "$kw_pro_name", "$new_baseline", "$view_root_kw\\bahama\\code\\annofile", "C:\\Klocwork\\Server_10_2_1", "zch49kw01", "8080", "D:\\KW_Tables", "no" );
